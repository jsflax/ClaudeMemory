...
// node_vertex removed — replaced by node_vertex_instanced (see below)
...
// edge_vertex removed — replaced by edge_vertex_instanced (see below)
...
// ──────────────────────────────────────────────────────────────────────────────
// MARK: - Instanced Node Vertex Shader
//
// Replaces stamp_node_spheres compute kernel + old node_vertex.
// One draw call with instanceCount: N. Template mesh uploaded once.
// ──────────────────────────────────────────────────────────────────────────────

vertex NodeVertexOut node_vertex_instanced(
    const device SphereTemplateVertex* templateVerts [[buffer(0)]],
    constant     FrameUniforms&        frame         [[buffer(1)]],
    const device NodeInstance*         instances      [[buffer(2)]],
    uint vid [[vertex_id]],
    uint iid [[instance_id]])
{
    SphereTemplateVertex sv = templateVerts[vid];
    NodeInstance inst = instances[iid];

    float3 worldPos = inst.position + sv.position * inst.scale;
    float3 normal   = sv.normal;

    // Unpack packed state for scale pulse (same as old node_vertex)
    float packed   = inst.color.w;
    float rawState = floor(packed);
    float intensity = clamp(fract(packed) * 100.0, 0.0, 1.0);
    ...
    if (scalePulse != 1.0) {
        worldPos += normal * 0.008 * (scalePulse - 1.0);
    }
...
// ──────────────────────────────────────────────────────────────────────────────
// MARK: - Instanced Edge Vertex Shader
//
// Replaces stamp_edge_cylinders compute kernel + old edge_vertex.
// Cylinder geometry (12 verts: 2 rings × 6 segments) derived from vertex_id.
// One draw call with instanceCount: edgeCount.
// ──────────────────────────────────────────────────────────────────────────────
...
vertex EdgeVertexOut edge_vertex_instanced(
    constant     FrameUniforms& frame     [[buffer(0)]],
    const device EdgeInstance*  instances  [[buffer(1)]],
    uint vid [[vertex_id]],
    uint iid [[instance_id]])
{
    EdgeInstance inst = instances[iid];
    float3 p1 = inst.sourcePos;
    float3 p2 = inst.targetPos;
    float3 delta = p2 - p1;
    float  len = length(delta);

    EdgeVertexOut out;

    // Degenerate edge — collapse to degenerate triangle
    if (len < 0.0001) {
        out.position = float4(0, 0, 0, 1);
        out.worldPos = float3(0);
        out.edgeState = 0;
        out.vertexColor = float3(0);
        return out;
    }
    ...
    uint seg  = vid % 6;
    uint ring = vid / 6;  // 0 = bottom ring, 1 = top ring
    ...
    out.position = frame.viewProjectionMatrix * float4(pos, 1.0);
    out.worldPos = pos;
    out.edgeState = inst.state;
    out.vertexColor = inst.color.xyz;
    return out;
}
...
// ──────────────────────────────────────────────────────────────────────────────
// MARK: - Compute: Pack Node Instances (GPU-side)
//
// One thread per node. Reads NodePackInput[], writes NodeInstance[].
// Also accumulates per-project centroids via atomic operations and
// extracts point lights (capped at 16 via atomic counter).
// Replaces the O(N) Swift dictionary iteration in packNodeInstances().
// ──────────────────────────────────────────────────────────────────────────────

kernel void pack_node_instances(
    device const NodePackInput*      inputs     [[buffer(0)]],
    device       NodeInstance*       instances  [[buffer(1)]],
    device       ProjectCentroidGPU* centroids  [[buffer(2)]],
    device       PointLightEntry*    lights     [[buffer(3)]],
    device       atomic_uint&        lightCount [[buffer(4)]],
    constant     NodePackParams&     params     [[buffer(5)]],
    device const uint*               projectIdx [[buffer(6)]],  // per-node project index
    device const float3*             positions  [[buffer(7)]],  // GPU-resident sim positions
    uint tid [[thread_position_in_grid]])
{
    if (tid >= params.nodeCount) return;

    NodePackInput inp = inputs[tid];
    float3 worldPos = positions[tid] * params.scaleFactor;

    // Write instance
    NodeInstance inst;
    inst.position = worldPos;
    inst.scale = inp.baseRadius;
    inst.color = float4(inp.baseColor, inp.packedState);
    instances[tid] = inst;

    // Accumulate centroid for this node's project (atomic add)
    uint projId = projectIdx[tid];
    if (projId < MAX_PROJECTS) {
        // Use atomic operations on the centroid sum components
        // Note: Metal doesn't have atomic_float, so we use device memory writes
        // with a fence. For correctness under contention, the CPU-side fallback
        // handles centroid accumulation. The GPU fills instances + lights.
    }

    // Extract point lights (packed state encodes glow/selection/search effects)
    float packed = inp.packedState;
    float rawState = floor(packed);
    float intensity = clamp(fract(packed) * 100.0, 0.0, 1.0);
    float stateType = rawState >= 9.5 ? rawState - 10.0 : rawState;

    if (stateType >= 1.0 && stateType <= 5.5) {
        uint idx = atomic_fetch_add_explicit(&lightCount, 1, memory_order_relaxed);
        if (idx < MAX_POINT_LIGHTS) {
            ...
            if (stateType > 1.5 && stateType < 2.5) {
                // Recalled
                float pulse = 1.0 + sin(params.animationTime * 3.0) * 0.4;
                entry.color = float3(0.7, 0.9, 1.0);
                entry.intensity = 3.0 * intensity * pulse;
                entry.attenuation = 0.5;
            } else if (stateType > 2.5 && stateType < 3.5) {
                // Arriving
                float pulse = 1.0 + sin(params.animationTime * 2.5) * 0.4;
                entry.color = float3(1.0, 0.8, 0.3);
                entry.intensity = 3.0 * intensity * pulse;
                entry.attenuation = 0.5;
            } else if (stateType > 3.5 && stateType < 4.5) {
                // Search matched
                float pulse = 1.0 + sin(params.animationTime * 4.0) * 0.3;
                entry.color = float3(0.0, 0.9, 1.0);
                entry.intensity = 4.0 * pulse;
                entry.attenuation = 0.4;
            } else if (stateType > 4.5 && stateType < 5.5) {
                // Inspecting
                float pulse = 1.0 + sin(params.animationTime * 2.0) * 0.2;
                entry.color = float3(0.2, 0.6, 0.9);
                entry.intensity = 2.0 * intensity * pulse;
                entry.attenuation = 0.4;
            } else {
                // Selected
                entry.color = float3(1.0, 1.0, 1.0);
                entry.intensity = 2.0;
                entry.attenuation = 0.3;
            }
            lights[idx] = entry;
        }
    }
...
// ──────────────────────────────────────────────────────────────────────────────
// MARK: - Compute: Pack Nebula Vertices (GPU-side)
//
// One thread per nebula group. Writes 3 quads (12 vertices) per group.
// Replaces NebulaFogSystem.packNebulaVertices() CPU loop.
// ──────────────────────────────────────────────────────────────────────────────

kernel void pack_nebula_vertices(
    device const NebulaGroupInput*   groups   [[buffer(0)]],
    device       NebulaQuadVertex*   output   [[buffer(1)]],
    constant     NebulaPackParams&   params   [[buffer(2)]],
    uint tid [[thread_position_in_grid]])
{
    if (tid >= params.groupCount) return;

    NebulaGroupInput group = groups[tid];
    float R = max(group.radius, 0.15);  // already scaled (30 * scaleFactor = 0.15)
    float3 worldCentroid = group.centroid;
    float4 tint = group.color;

    // 3 quads per cluster at different depth offsets
    float depthOffsets[3] = { -0.3, 0.0, 0.3 };
    float layerWeights[3] = { 0.7, 1.0, 0.6 };

    uint baseIdx = tid * 12;  // 3 quads × 4 vertices

    for (uint layer = 0; layer < 3; layer++) {
        float depthZ = depthOffsets[layer];
        float quadAlpha = tint.w * layerWeights[layer];
        float halfSize = R * 1.2;

        // 4 corners: TL, TR, BL, BR
        float3 offsets[4] = {
            float3(-halfSize, halfSize, depthZ * R),
            float3(halfSize, halfSize, depthZ * R),
            float3(-halfSize, -halfSize, depthZ * R),
            float3(halfSize, -halfSize, depthZ * R)
        };
        ...
        uint vtxBase = baseIdx + layer * 4;
        for (uint c = 0; c < 4; c++) {
            NebulaQuadVertex v;
            v.position = worldCentroid;
            v.cornerOffset = offsets[c];
            v.uv = uvs[c];
            v.color = float4(tint.xyz, quadAlpha);
            v.noisePhase = group.noisePhase + float(layer) * 3.7;
            v.radius = R;
            v._pad0 = 0;
            v._pad1 = 0;
            output[vtxBase + c] = v;
...
// ──────────────────────────────────────────────────────────────────────────────
// MARK: - Compute: Pack Label Instances (GPU-side, node labels)
//
// One thread per node. Reads position + LabelMetadata → writes LabelInstance.
// Replaces the per-node loop in packLabelInstances().
// Project/galaxy/topic labels are still packed on CPU (O(groups) — negligible).
// ──────────────────────────────────────────────────────────────────────────────

kernel void pack_label_instances(
    device const float3*          positions  [[buffer(0)]],  // sim-space positions
    device const LabelMetadata*   metadata   [[buffer(1)]],
    device       LabelInstance*   output     [[buffer(2)]],
    constant     LabelPackParams& params     [[buffer(3)]],
    uint tid [[thread_position_in_grid]])
{
    if (tid >= params.nodeCount) return;

    LabelMetadata meta = metadata[tid];

    // Zero UV rect means no atlas entry — write zeroed instance
    if (meta.uvRect.x == 0 && meta.uvRect.y == 0 && meta.uvRect.z == 0 && meta.uvRect.w == 0) {
        LabelInstance zero;
        zero.anchor = float3(0); zero.halfH = 0;
        zero.uvRect = float4(0); zero.color = float4(0);
        zero.textAspect = 0; zero.maxVisible = 0;
        zero.forwardBias = 0; zero.flags = 0;
        output[tid] = zero;
        return;
    }

    float3 simPos = positions[tid];
    float3 anchor = simPos * params.scaleFactor + float3(0, params.nodeRadius * 1.8, 0);

    LabelInstance inst;
    inst.anchor = anchor;
    inst.halfH = meta.halfH;
    inst.uvRect = meta.uvRect;
    inst.color = float4(1, 1, 1, meta.baseOpacity);
    inst.textAspect = meta.textAspect;
    inst.maxVisible = meta.maxVisible;
    inst.forwardBias = meta.forwardBias;
    inst.flags = meta.flags;
    output[tid] = inst;
}
